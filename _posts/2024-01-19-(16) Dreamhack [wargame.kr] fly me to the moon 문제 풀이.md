---
title: (16) Dreamhack [wargame.kr] fly me to the moon 문제 풀이
...
## 문제 풀이
### 쉬운 풀이
![](https://kyuyeop.github.io/assets/img/post/16/1.png)
먼저 사이트에 들어오면 이러한 게임 사이트가 나옵니다. 마우스로 조작하는 게임인데, 게임오버가 되면
![](https://kyuyeop.github.io/assets/img/post/16/2.png)
31337점을 따야한다고 합니다.~~(심심하시면 그냥 직접 해봐도)~~  

암튼, 그냥 하면 한참 걸릴테니, 허점을 찾아야 합니다. js로 만들어졌다고 하니 코드를 한번 봐봅시다.
```javascript
...
```
윽, 엄청나게 긴 코드가 있고, 난독화 되어 있습니다.  
  
게임을 한번 실행하고 콘솔을 보면 친절하게 해독된 코드를 볼 수 있게 해줍니다.
![](https://kyuyeop.github.io/assets/img/post/16/3.png)
파랗게 하이퍼링크 처리된 곳을 누르면 아래와 같은 해독된 코드를 볼 수 있습니다.
```javascript
...
;
```
여전히 알아보기 더럽게 힘듭니다. 그래도 전보다는 보기 좋네요.  
  
그리고 혹시나 뭔가 통신하는게 있는가 싶어서 네트워크 탭도 확인해봤습니다.
![](https://kyuyeop.github.io/assets/img/post/16/4.png)
보면 게임오버가 되면 high-scores.php에 POST요청을 날립니다.  
페이로드 창을 확인해보면
![](https://kyuyeop.github.io/assets/img/post/16/5.png)
token과 score를 전달해 주고 있습니다. 그래서 혹시나 하고 이전에 있는 token.php로 보낸 요청을 확인해봤습니다.
![](https://kyuyeop.github.io/assets/img/post/16/6.png)
앞서 확인한 token 값이 그대로 있습니다.  
  
이를 통해 안티치트가 어떻게 작동하는지 알았습니다.  
보면 약 10초마다 token.php로 부터 token을 생성합니다. 그리고 해당 토큰과 점수를 함께 보내는 방식으로 안티치트가 동작합니다.  
  
그래서 그냥 burp suite를 켜고 token 발행 간격 안에 점수를 수정해서 보내면 됩니다.
![](https://kyuyeop.github.io/assets/img/post/16/7.png)
이렇게 간단하게 풀어도 flag가 나옵니다.  
  
### 살짝 어려운 풀이
자바스크립트를 수정하는 방식으로도 문제를 풀 수 있습니다.  
  
일단 점수를 나타내는 변수가 뭔지부터 알아내야 겠죠? 점수가 1씩 늘어나니까 ++나 +1 같은 문자열을 탐색해보면 알수 있을거고, 아니면 post요청을 보내는 부분을 찾아도 되겠죠. 실제로 찾아보면
```javascript
function _0x8618x8() {
    if (_0x8618x3) {
        _0x8618x6++
...
아마도 `_0x8618x6`가 점수를 담은 변수로 보여집니다. post요청을 보내는 부분은 `BTunnelGame[_0x32bb[2]]()`로 되어 있지만 쉽게 변조가 가능해 보이네요.

`BTunnelGame[_0x32bb[2]]()` 부분에 대해 조금 더 설명해 보겠습니다.
먼저 저게 뭘 나타내는지 알아야 겠죠. 참고로 `_0x32bb`의 3번째 값은 `"\x67\x65\x74\x53\x63\x6F\x72\x65"`인데, hex-ascii 변환해보면 `"getScore"`입니다. 점수 가져오는 함수라네요.  
...
```
이렇게 정의가 있습니다. `_0x8618x7` 함수도 살펴보겠습니다.
```javascript
function _0x8618x7() {
    return _0x8618x6
}
```
앞서 찾은 점수 변수인 `_0x8618x6`가 반환되는 모습입니다. 이로써 `_0x8618x6`가 확실히 점수임을 알 수 있네요.  
  
  
따라서 아래와 같이 점수 초기값을 바꾸거나
```javascript
var _0x8618x6 = 31337;
```
![](https://kyuyeop.github.io/assets/img/post/16/8.png)
점수에 1씩이 아니라 아주 큰 수를 더해버리거나
```javascript
function _0x8618x8() {
    if (_0x8618x3) {
        _0x8618x6+=100000
...
```
![](https://kyuyeop.github.io/assets/img/post/16/9.png)
아니면 무조건 31337을 반환하도록 만들어도 됩니다.
```javascript
} else {
    $(_0x32bb[30])[_0x32bb[39]](_0x32bb[31]);
    $(_0x32bb[19])[_0x32bb[15]](_0x32bb[16], _0x32bb[24]);
...
```
![](https://kyuyeop.github.io/assets/img/post/16/10.png)
